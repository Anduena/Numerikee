...
public class Metoda_SOR {
	public static double[] sor(double[][] A, double w, double[] x0, double tol, int N) {
		int n = A.length;
		int k = 1;
		while (k <= N) {
			...
			for (int i = 0; i < n; i++) {
				x[i] = (1 - w) * x0[i]
						+ (w * (-shuma(0, i - 1, A, x, i) - shuma(i + 1, n - 1, A, x0, i) + A[i][n])) / A[i][i];
			}
