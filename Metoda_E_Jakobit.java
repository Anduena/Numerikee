...
public class Metoda_E_Jakobit {
	public static double[] jakobi(double[][] A, double[] x0, double tol, int N) {
		int n = A.length;
		int k = 1;
		while (k <= N) {
			double[] x = new double[n];
			for (int i = 0; i < n; i++) {
				x[i] = (-shuma(n, A, x0, i) + A[i][n]) / A[i][i];
			}
	...
	public static double shuma(int n, double[][] A, double[] x0, int i) {
		double shuma = 0;
		for (int j = 0; j < n; j++) {
			if (j != i) {
				shuma += A[i][j] * x0[j];
